...
if [ "$KERNEL_TYPE" = "tcpaad" ]; then
    ...
    RESULTS_DIR="${RESULTS_BASE}/main_test/${KERNEL_TYPE}"
    mkdir -p "${RESULTS_DIR}"/{logs,raw,dmesg}
fi
SESSION_LOG="${RESULTS_BASE}/main_test/session.log"
mkdir -p "${RESULTS_BASE}/main_test"
...
if [ "$KERNEL_TYPE" = "tcpaad" ]; then
    RESULTS_DIR="${RESULTS_BASE}/main_test/${KERNEL_TYPE}_${ALPHA}"
    mkdir -p "${RESULTS_DIR}"/{logs,raw,dmesg}
fi
