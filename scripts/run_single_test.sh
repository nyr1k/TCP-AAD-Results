...
run_single_test() {
  ...
  if ! output=$(tc_cleanup "${CLIENT_IFACE}" 2>&1)
  then
    log_error "tc cleanup failed before ${test_base}: ${output}" "$test_log"
    return 1
  fi
  
  if ! output=$(tc_setup "${bw}" "${delay}" "${CLIENT_IFACE}" 2>&1)
  then
    log_error "tc setup failed for bw=${bw} delay=${delay}: ${output}" "$test_log"
    return 1
  fi
  ...
  # Write test header
  {
      ...
      echo "Server kernel: ${SERVER_KERNEL}"
      echo "rate=${CURRENT_RATE}  bw=${bw}  delay=${delay}  duration=${duration}s  iterations=${iterations}"
      echo "================================================================================"
  } > "${test_log}"

  local -a iperf_opts=(-c "${SERVER_IP}" -p "${IPERF_PORT}" -t "${duration}" -i 1 -J --get-server-output -N)


  for ((i = 1; i <= iterations; i++)); do

    rm -f /tmp/server1.ready
    rm -f /tmp/server2.ready
    rm -f /tmp/start
  ...
  # Create the local ftrace output directory if it doesn't exist
    ...
    server_ssh "sudo sh -c 'echo 0 > /sys/kernel/debug/tracing/tracing_on && echo > /sys/kernel/debug/tracing/trace && echo 1 > /sys/kernel/debug/tracing/tracing_on'"

    echo "${pcap_file}"

    # Start pcap
    if ! output=$(server_ssh "$(pcap_start_cmd "$pcap_file" "$SERVER_IFACE" "$SERVER_IP" "$CLIENT_IP")" 2>&1); then
    ...
    touch /tmp/server1.ready

    timeout=300

    while [[ ! -f /tmp/server2.ready ]]; do
        ((timeout--))
        if ((timeout==0)); then
            log_error "Timed out waiting for server2"
            exit 1
        fi
    ...
    server_ssh "sudo sh -c 'echo 0 > /sys/kernel/debug/tracing/tracing_on'"
    
    # Safely pull the trace stream over SSH, extracting only your active research tokens
    server_ssh "sudo cat /sys/kernel/debug/tracing/trace" | grep -i "iperf" > "${ftrace_local_file}"

    # Stop pcap, count ACKs, cleanup (single SSH call)
    local ack_count
    if ! ack_count=$(pcap_stop_count_cleanup "$pcap_file" 2>&1)
    then
    ...
    if ! output=$(tc_cleanup "${CLIENT_IFACE}" 2>&1)
    then
        log_error "tc cleanup failed after ${test_base}: ${output}" "$test_log"
    fi
